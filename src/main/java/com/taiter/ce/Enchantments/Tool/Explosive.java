...
public class Explosive extends CEnchantment {
  ...
  public void effect(Event e, ItemStack item, int level) {
    ...
    if (!isUsable(player.getItemInHand().getType().toString(),
    ...
    for (int x = 0; x < locRad; x++) {
      for (int y = 0; y < locRad; y++) {
        for (int z = 0; z < locRad; z++) {
          if ((!(x == 0 && y == 0 && z == 0)) && (!(x == r && y == 0 && z == 0)) && (!(x == 0
              && y == r && z == 0)) && (!(x == 0 && y == 0 && z == r)) && (!(x == r && y == r
              && z == 0))
              && (!(x == 0 && y == r && z == r)) && (!(x == r && y == 0 && z == r)) && (!(x == r
              && y == r && z == r))) {
  ...
  private boolean isUsable(String iMat, String bMat) {
    return (iMat.endsWith("PICKAXE") && (bMat.contains("ORE") || (!bMat.contains("STAIRS")
        && bMat.contains("STONE")) || bMat.equals("STAINED_CLAY") || bMat.equals("NETHERRACK")))
        || (iMat.endsWith("SPADE") && (bMat.contains("SAND") || bMat.equals("DIRT") || bMat.equals(
        "SNOW_BLOCK") || bMat.equals("SNOW") || bMat.equals("MYCEL") || bMat.equals("CLAY")
        || bMat.equals("GRAVEL") || bMat.equals("GRASS")))
        || (iMat.endsWith("_AXE") && bMat.contains("LOG") || bMat.contains("PLANKS")) || (
        iMat.endsWith("HOE") && (bMat.equals("CROPS") || bMat.equals("POTATO") || bMat.equals(
            "CARROT")));
  }
  ...
  public void initConfigEntries() {
    Radius = Integer.parseInt(
        getConfig().getString("Enchantments." + getOriginalName() + ".Radius"));
    if (Radius % 2 == 0) {
      Radius += 1;
    }
    LargerRadius = Boolean.parseBoolean(
        getConfig().getString("Enchantments." + getOriginalName() + ".LargerRadius"));
    DropItems = Boolean.parseBoolean(
        getConfig().getString("Enchantments." + getOriginalName() + ".DropItems"));
  }

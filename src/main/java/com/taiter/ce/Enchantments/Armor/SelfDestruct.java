...
public class SelfDestruct extends CEnchantment {
  ...
  public void effect(Event e, ItemStack item, int level) {
    ...
    for (int i = level; i >= 0; i--) {
      TNTPrimed tnt = (TNTPrimed) event.getEntity().getWorld()
          .spawnEntity(event.getEntity().getLocation(), EntityType.PRIMED_TNT);
      tnt.setFuseTicks(delay);
      tnt.setVelocity(
          new Vector(Tools.random.nextDouble() * 1.5 - 1, Tools.random.nextDouble() * 1.5,
              Tools.random.nextDouble() * 1.5 - 1));
  ...
  public void initConfigEntries() {
    delay = Integer.parseInt(
        getConfig().getString("Enchantments." + getOriginalName() + ".ExplosionDelay"));
  }
}

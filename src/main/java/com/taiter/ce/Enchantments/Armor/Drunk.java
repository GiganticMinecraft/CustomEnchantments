...
public class Drunk extends CEnchantment {
  ...
  public void effect(Event e, ItemStack item, int level) {
    ...
    target.addPotionEffect(
        new PotionEffect(PotionEffectType.SLOW, duration * level, strength + level));
    target.addPotionEffect(
        new PotionEffect(PotionEffectType.SLOW_DIGGING, duration * level, strength + level));
  ...
  public void initConfigEntries() {
    duration = Integer.parseInt(
        getConfig().getString("Enchantments." + getOriginalName() + ".DurationPerLevel"));
    strength = Integer.parseInt(
        getConfig().getString("Enchantments." + getOriginalName() + ".BaseStrength")) - 1;
  }
}

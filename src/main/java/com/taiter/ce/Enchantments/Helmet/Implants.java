...
public class Implants extends CEnchantment {
    ...
    if (air < event.getPlayer().getMaximumAir()) {
      if (!inWater.contains(player)) {
        ...
        new BukkitRunnable() {
          @Override
          public void run() {
            if (player.isOnline() && !player.isDead() && player.getInventory().getBoots() != null
                && player.getInventory().getBoots().hasItemMeta() && player.getInventory()
                .getBoots().getItemMeta().hasLore() && player.getInventory().getBoots()
                .getItemMeta().getLore().contains(getDisplayName())) {
  ...
  public void initConfigEntries() {
    this.burstDelay = Integer.parseInt(
        Main.config.getString("Enchantments." + getOriginalName() + ".FoodBurstDelay"));
  }
}

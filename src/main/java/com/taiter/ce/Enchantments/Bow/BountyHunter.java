...
public class BountyHunter extends CEnchantment {
  ...
  public void effect(Event e, ItemStack item, final int level) {
    if (e instanceof EntityDamageByEntityEvent) {
      ...
      // なにをしているのかが分からなかったのでコメントアウトをした。
      // 具体的には、Effect.COLOURED_DUSTがSpigot 1.13以降で色を指定するようになったが、どの色をつければよいか分からなかった。
//		for(int i = 10; i>0; i--) {
...
//		}
      ...
      p.getInventory()
          .addItem(new ItemStack(bountyDrop, Tools.random.nextInt(MaximumBounty + level) + 1));
  ...
  public void initConfigEntries() {
    MaximumBounty = Integer.parseInt(
        getConfig().getString("Enchantments." + getOriginalName() + ".MaximumBounty"));
    Cooldown = Integer.parseInt(
        getConfig().getString("Enchantments." + getOriginalName() + ".Cooldown"));
    ChanceEmerald = Double.parseDouble(
        getConfig().getString("Enchantments." + getOriginalName() + ".ChanceEmerald"));
    ChanceDiamond = Double.parseDouble(
        getConfig().getString("Enchantments." + getOriginalName() + ".ChanceDiamond"));
    ChanceGold = Double.parseDouble(
        getConfig().getString("Enchantments." + getOriginalName() + ".ChanceGold"));
    ChanceIron = Double.parseDouble(
        getConfig().getString("Enchantments." + getOriginalName() + ".ChanceIron"));
  }

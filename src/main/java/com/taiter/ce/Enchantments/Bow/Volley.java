...
public class Volley extends CEnchantment {
  ...
  private void volley(EntityShootBowEvent e, ItemStack item, int lvl) {
    ...
    for (int i = 0; i < amount;
        i++) { // spawn all arrows in a cone of 90 degrees (equally distributed).;
      ...
      arrow.setShooter(p);
      arrow.setVelocity(newDir.normalize().multiply(oldArrow.getVelocity()
          .length())); // Need to make sure arrow has same speed as original arrow.
      ...
      if (i == 0) {
        if (p.getGameMode().equals(GameMode.CREATIVE)) {
          arrow.setMetadata("ce.Volley", new FixedMetadataValue(getPlugin(),
              null)); //Control metadata to prevent players from duplicating arrows
        }
      } else {
        arrow.setMetadata("ce.Volley", new FixedMetadataValue(getPlugin(),
            null)); //Control metadata to prevent players from duplicating arrows

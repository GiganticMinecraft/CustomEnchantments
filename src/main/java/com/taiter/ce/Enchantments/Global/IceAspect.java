  ...
  public void effect(Event e, ItemStack item, int level) {
    ...
    if (i < chanceFreeze) {
      ((LivingEntity) event.getEntity()).addPotionEffect(
          new PotionEffect(PotionEffectType.SLOW, SlowDuration, SlowStrength, false), true);
    ...
    if (specialFreeze) {
      if (i < chanceSpecialFreeze) {
        if (event.getEntity() instanceof LivingEntity) {
          ...
          ent.addPotionEffect(
              new PotionEffect(PotionEffectType.SLOW, SpecialFreezeDuration + 20, 10));
...
//            b.getKey().setData((byte) Integer.parseInt(b.getValue().split(" ")[1]));
      ...
      for (int y = by - 1; y <= by + size; y++) {
        for (int z = bz - size; z <= bz + size; z++) {
          ...
          if (distancesquared < (size * size) && distancesquared >= ((size - 1) * (size - 1))) {
            ...
            if ((b.getType() == Material.AIR || (!b.getType().equals(Material.CARROT)
                && !b.getType().equals(Material.POTATO) && !b.getType().equals(Material.WHEAT)
                && !b.getType().toString().contains("SIGN") && !b.getType().isSolid()))
                && Tools.checkWorldGuard(b.getLocation(), p, "PVP", false)) {
              list.put(b, b.getType() + " " + b.getData());
  ...
  public void initConfigEntries() {
    SlowStrength = Integer.parseInt(
        getConfig().getString("Enchantments." + getOriginalName() + ".SlowStrength"));
    SlowDuration = Integer.parseInt(
        getConfig().getString("Enchantments." + getOriginalName() + ".SlowDuration"));
    chanceFreeze = Integer.parseInt(
        getConfig().getString("Enchantments." + getOriginalName() + ".ChanceFreeze"));
    SpecialFreezeDuration = Integer.parseInt(
        getConfig().getString("Enchantments." + getOriginalName() + ".SpecialFreezeDuration"));
    chanceSpecialFreeze = Integer.parseInt(
        getConfig().getString("Enchantments." + getOriginalName() + ".ChanceSpecialFreeze"));
    specialFreeze = Boolean.parseBoolean(
        getConfig().getString("Enchantments." + getOriginalName() + ".SpecialFreeze"));
  }
}

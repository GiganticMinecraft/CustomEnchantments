...
public abstract class CEnchantment extends CBasic {
  ...
  public CEnchantment(Application app) {
    ...
    for (int i = 3; i < nameChars.length;
        i++) { //Go through the classname, start at the third char (Minimum enchantment length) and check for an uppercase letter
        if (Character.isUpperCase(nameChars[i])) {
            this.originalName =
                originalName.substring(0, i) + " " + originalName.substring(i, nameChars.length);
        }
    }
    ...
    this.configEntries.put("RunecraftingCost",
        Arrays.asList("0LVL&0$", "0LVL&0$", "0LVL&0$", "0LVL&0$", "0LVL&0$"));
  }
  ...
  public void finalizeEnchantment() {
    ...
    try {
      this.displayName =
          EnchantManager.getLorePrefix() + ChatColor.translateAlternateColorCodes('&',
              Main.config.getString("Enchantments." + getOriginalName() + ".DisplayName"));
        if (!Boolean.parseBoolean(
            getConfig().getString("Global.Enchantments.UseCustomEnchantmentProbability"))) {
            this.enchantProbability = enchantmentProbability;
        } else {
            this.enchantProbability = Double.parseDouble(
                Main.config.getString(
                    "Enchantments." + getOriginalName() + ".EnchantmentProbability"));
        }

        if (this.enchantmentMaxLevel != -1) {
            this.enchantmentMaxLevel = Integer.parseInt(
                Main.config.getString(
                    "Enchantments." + getOriginalName() + ".EnchantmentMaxLevel"));
        } else {
            this.enchantmentMaxLevel = 1;
        }
      this.occurrenceChance = Integer.parseInt(
          Main.config.getString("Enchantments." + getOriginalName() + ".OccurrenceChance"));
      ...
      if (costPerLevel.isEmpty()) {
        double cost = Double.parseDouble(
            getConfig().getString("Enchantments." + getOriginalName() + ".Cost"));
      ...
      List<String> list = getConfig().getStringList(
          "Enchantments." + getOriginalName() + ".RunecraftingCost");

      if (list.isEmpty()) {
        String cost = getConfig().getString(
            "Enchantments." + getOriginalName() + ".RunecraftingCost");
          if (cost == null) {
              cost = "0LVL&0$";
          }
      ...
      if (!hasRetriedConfig) {
        ...
        Bukkit.getConsoleSender().sendMessage(
            ChatColor.RED + "[CE] WARNING: Could not configurate the CE '" + getOriginalName()
                + "',");
        Bukkit.getConsoleSender().sendMessage(ChatColor.RED
            + "[CE]          please check the config for any errors, the enchantment is now disabled. ");

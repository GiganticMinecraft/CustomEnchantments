...
public class EnchantManager {
  ...
  private static final Set<CEnchantment> enchantments = new LinkedHashSet<CEnchantment>();
  ...
  private static Enchantment registerGlowEnchantment() {
    ...
    Enchantment glow = Enchantment.getByKey(NamespacedKey.fromString(String.valueOf(id)));
    if (glow != null) {
...
//            else
//                id = Enchantment.values()[Enchantment.values().length - 1] + 1;
  ...
  /**
   * Retrieves an enchantment by its original name. Assumes that the given String is not colored and
   * equals the name of an enchantment.
   *
   * @param originalName The original name of the enchantment to retrieve
      ...
      for (String name : lore) {
        if (name.length() > 3) {
          for (CEnchantment ce : enchantments) {
            String enchantment = ChatColor.stripColor(ce.getDisplayName()).toLowerCase();
            name = ChatColor.stripColor(name).toLowerCase();
            if (name.startsWith(enchantment) || name.startsWith(
                ce.getOriginalName().toLowerCase())) {
              String[] split = name.split(" ");
              name = name.substring(0, name.length() - 1 - split[split.length - 1].length());
              if (name.equals(enchantment) || name.equals(ce.getOriginalName())) {
                list.add(ce);
  ...
  public static boolean isEnchantmentBook(ItemStack i) {
    if (i != null && i.getType().equals(Material.ENCHANTED_BOOK)) {
      return i.hasItemMeta() && i.getItemMeta().hasDisplayName() && i.getItemMeta().getDisplayName()
          .equals(enchantBookName);
    }
    return false;
  }
  ...
  public static boolean isEnchantable(String mat) {
    if (mat.contains("HELMET") || mat.contains("CHESTPLATE") || mat.contains("LEGGINGS")
        || mat.contains("BOOTS") || mat.contains("SWORD") || mat.contains("PICKAXE")
        || mat.contains("AXE")
    ...
    return (Main.config.getBoolean("Global.Runecrafting.Disenchanting") && mat.equals("BOOK"))
        || ((Main.config.getBoolean("Global.Runecrafting.CanStackEnchantments") && mat.equals(
        "ENCHANTED_BOOK")));
  }
  ...
  public static Boolean containsEnchantment(String toTest, CEnchantment ce) {
    if (toTest.startsWith(ChatColor.YELLOW + String.valueOf(ChatColor.ITALIC) + "\"")) {
  ...
  public static ItemStack getEnchantBook(CEnchantment ce, int level) {
    ...
    ItemMeta im = item.getItemMeta();
    im.setLore(
        Arrays.asList(lorePrefix + ce.getDisplayName() + " " + intToLevel(level)));

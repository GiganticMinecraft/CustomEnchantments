...
public class Tools {
  ...
  public static boolean isApplicationCorrect(Application app, Material matToApplyTo) {
    ...
    if (app == Application.BOW && mat.equals(Material.BOW.toString())) {
      ...
      return true;
    } else if (app == Application.ARMOR && (mat.endsWith("HELMET") || mat.endsWith("CHESTPLATE")
        || mat.endsWith("LEGGINGS") || mat.endsWith("BOOTS"))) {
      return true;
    } else {
      return app == Application.TOOL && (mat.endsWith("PICKAXE") || mat.endsWith("SPADE")
          || mat.endsWith("AXE") || mat.endsWith("HOE"));
    }
  }
  ...
  public static Inventory getPreviousInventory(String name) {
    if (name.equals(prefix + "Enchantments") || name.equals(prefix + "Items") || name.equals(
        prefix + "Config")) {
      return Main.CEMainMenu;
    } else if (name.equals(prefix + "Enchanting") || name.equals(prefix + "Armor") || name.equals(
        prefix + "Bow") || name.equals(prefix + "Tool") || name.equals(prefix + "Global")
        || name.equals(prefix + "Helmet") || name.equals(prefix + "Boots") || name.equals(
  ...
  public static Inventory getEnchantmentMenu(Player p, String name) {
    if (!p.isOp() && !p.hasPermission("ce.ench.*")) {
      ...
      Inventory enchantments = Bukkit.createInventory(null, lInv.getSize(), name);
  ...
  public static Inventory getItemMenu(Player p, String title) {
    if (!p.isOp() && !p.hasPermission("ce.item.*")) {
      Inventory lInv = Main.CEItemMenu;
      Inventory items = Bukkit.createInventory(null, lInv.getSize(), title);
  ...
  public static void generateInventories() {
    ...
    String itemPrefix = ChatColor.AQUA + String.valueOf(ChatColor.BOLD);
  ...
  public static void convertOldConfig() {
    Main.plugin.getConfig().set("Global.Enchantments.CEnchantmentColor",
        (Boolean.parseBoolean(Main.config.getString("enchantments.lore.disableItalic")) ? ""
            : "ITALIC;") + ChatColor.valueOf(Main.config.getString("enchantments.lore.color")));
    Main.plugin.getConfig().set("Global.Enchantments.CEnchantmentTable",
        Boolean.parseBoolean(Main.config.getString("enchantmentTable")));
    Main.plugin.getConfig().set("Global.Enchantments.CEnchantingProbability",
        Integer.parseInt(Main.config.getString("enchantmentTableProbability")));
    Main.plugin.getConfig().set("Global.Enchantments.MaximumCustomEnchantments",
        Integer.parseInt(Main.config.getString("maximumEnchants")));
  ...
  public static void writeConfigEntries(CBasic ce) {
    for (String entry : ce.configEntries.keySet()) {
      String fullPath = (ce.getType() == "Enchantment" ? "Enchantments" : ce.getType()) + "."
          + ce.getOriginalName() + "." + entry;
  ...
  public static boolean checkWorldGuard(Location l, Player p, String fs, boolean sendMessage) {
    ...
    if (Main.getWorldGuard() != null) {
      return WorldGuard.getInstance()
          ...
          .getRegionContainer()
          .createQuery()
          .testState(BukkitAdapter.adapt(l), Main.getWorldGuard().wrapPlayer(p), Flags.BUILD);
    }
    return true;
  }
  ...
  public static HashSet<CEnchantment> getEnchantList(Application app, Player p) {
    ...
    for (CEnchantment ce : EnchantManager.getEnchantments()) {
      if (ce.getApplication() == app) {
        if (!Boolean.parseBoolean(Main.config.getString("Global.Enchantments.RequirePermissions"))
            || checkPermission(ce, p)) {
          list.add(ce);
        }
      }
    }
  ...
  public static Application getApplicationByMaterial(Material material) {
    ...
    } else if (mat.endsWith("BOOTS") || mat.endsWith("LEGGINGS") || mat.endsWith("CHESTPLATE")
        || mat.endsWith("HELMET")) {
      return Application.ARMOR;
    } else if (mat.endsWith("PICKAXE") || mat.endsWith("SPADE") || mat.endsWith("AXE")
        || mat.endsWith("HOE")) {
  ...
  public static boolean isApplicable(ItemStack i, CEnchantment ce) {
    return (ce.getApplication() == Application.ARMOR && ce.getApplication() != Application.GLOBAL
        && (i.getType().toString().endsWith("HELMET") || i.getType().toString()
        .endsWith("CHESTPLATE") || i.getType().toString().endsWith("LEGGINGS")
        || i.getType().toString().endsWith("BOOTS")))
        || (ce.getApplication() == Application.TOOL && (i.getType().toString().endsWith("PICKAXE")
        || i.getType().toString().endsWith("SPADE") || i.getType().toString().endsWith("_AXE")
        || i.getType().toString().endsWith("HOE")))
        || (ce.getApplication() == Application.HELMET && ce.getApplication() != Application.GLOBAL
        && i.getType().toString().endsWith("HELMET"))
        || (ce.getApplication() == Application.BOOTS && ce.getApplication() != Application.GLOBAL
        && i.getType().toString().endsWith("BOOTS"))
  ...
  public static Firework shootFirework(Location loc, Random rand) {
    ...
    FireworkEffect effect = FireworkEffect.builder().flicker(rand.nextBoolean())
        .withColor(fireworkColor(rand.nextInt(16) + 1))
        .withFade(fireworkColor(rand.nextInt(16) + 1))

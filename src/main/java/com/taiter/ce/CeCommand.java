...
public class CeCommand {
  ...
  public String processCommand(CommandSender sender, String[] args) {
    ...
    String Success = String.valueOf(ChatColor.GREEN);
    String Error = String.valueOf(ChatColor.RED);
    ...
    if (args.length >= 1) {
      ...
      if (name.equals("reload")) {
        ...
        requiredPermission += "reload";
          if (!sender.hasPermission(node) && !sender.hasPermission(requiredPermission)
              && !sender.isOp()) {
        ...
        EnchantManager.setMaxEnchants(Integer.parseInt(
            Main.config.getString("Global.Enchantments.MaximumCustomEnchantments")));

        //Set the Loreprefix
        EnchantManager.setLorePrefix(Main.resolveEnchantmentColor());

        EnchantManager.setEnchantBookName(ChatColor.translateAlternateColorCodes('&',
            Main.config.getString("Global.Books.Name")));
          ...
          if (args.length >= 4) {
              ...
              for (CItem ci : Main.items) {
                  if (fullString.contains(ci.getOriginalName().toLowerCase())) {
                      ...
                      ci.getOriginalName().replace(" ", "").toLowerCase())) {
                      custom = ci;
                      ...
                      ci.getDisplayName().replace(" ", "").toLowerCase())) {
                      custom = ci;
              ...
              for (int i = 0; i < fullString.split(" ").length; i++) {
                  for (CEnchantment ce : EnchantManager.getEnchantments()) {
                      ...
                      if (fullString.contains(ce.getOriginalName().toLowerCase())) {
                          ...
                          endIndex = index + enchName.length()
                              + 2; // Making a substring from index - endIndex returns the enchantment's name with the level
                          ...
                          ce.getOriginalName().replace(" ", "").toLowerCase())) {
                          ...
                          ce.getDisplayName().replace(" ", "").toLowerCase())) {
                  ...
                  for (Enchantment e : Enchantment.values()) {
                      ...
                      if (fullString.contains(enchName)) {
                          ...
                          index = fullString.indexOf(enchName);
                          endIndex = index + enchName.length()
                              + 3; // Making a substring from index - endIndex returns the enchantment's name with the level
              ...
              String targetNotification = String.valueOf(ChatColor.GOLD);
              ...
              if (custom != null) {
                  if (Tools.checkPermission(custom, target)) {
                      ...
                      targetNotification +=
                          "You have received an enchanted item from " + sender.getName() + "!";
                  } else {
                      Error += target.getName() + " does not have the permission to use the item "
                          + custom.getOriginalName() + ".";
                      return Error;
                  }
              }
              ...
              if (!enchants.isEmpty()) {
                  ...
                  if (Success.length() < 10) {
                      Success +=
                          "The enchanted Item was successfully given to Player " + target.getName()
                              + ".";
                      targetNotification +=
                          "You have received an enchanted item from " + sender.getName() + "!";
                  }
              ...
              if (!cEnchants.isEmpty()) {
                  ...
                  for (String e : cEnchants) {
                      ...
                      list.put(EnchantManager.getEnchantment(e),
      ...
      if (sender instanceof Player) {
        ...
        if (name.startsWith("rune")) {
          ...
          inv.setContents(new ItemStack[]{new ItemStack(Material.AIR), new ItemStack(Material.AIR),
            ...
            CEnchantment ce = EnchantManager.getEnchantment(args[1]);
              ...
              for (String s : im.getLore()) {
                  if (EnchantManager.containsEnchantment(s, ce)) {
                      ...
                      im.setLore(lore);
                      item.setItemMeta(im);
                      return Success + "Removed the enchantment " + ce.getDisplayName()
                          + ChatColor.GREEN
                          + "!";
                  }
              }
        ...
        if (name.startsWith("i") || name.startsWith("e")) {
          ...
          usageError += (name.startsWith("e") ? "enchant [Required Material] <Enchantment> <Level>"
              : "item <Item>");
            if (args.length >= 2) {

                String customName = args[1];
                Material test = Material.getMaterial(customName);
                ...
                if (name.startsWith("e")) {
                    for (CEnchantment ce : EnchantManager.getEnchantments()) {
                        if (ce.getOriginalName().equalsIgnoreCase(customName)
                            || ChatColor.stripColor(
                            ce.getDisplayName()).equalsIgnoreCase(customName)
                            || ce.getOriginalName().replace(" ", "").equalsIgnoreCase(customName)
                            || ChatColor.stripColor(ce.getDisplayName()).replace(" ", "")
                            .equalsIgnoreCase(customName)) {
                            custom = ce;
                    ...
                    for (CItem ci : Main.items) {
                        if (ci.getOriginalName().equalsIgnoreCase(customName)
                            || ChatColor.stripColor(
                            ci.getDisplayName()).equalsIgnoreCase(customName)
                            || ci.getOriginalName().replace(" ", "").equalsIgnoreCase(customName)
                            || ChatColor.stripColor(ci.getDisplayName()).replace(" ", "")
                            .equalsIgnoreCase(customName)) {
                            custom = ci;
                        }
                    }
                ...
                if (custom == null) {
                    ...
                    if (ench != null) {
                        if (item.containsEnchantment(ench)) {
                            ...
                            return Success + "You have succesfully increased the item's level of "
                                + ench.getName() + " by " + level + ".";
                        } else {
                            item.addUnsafeEnchantment(ench, level);
                            return Success + "You have succesfully enchanted your item with "
                                + ench.getName()
                                + " level " + level + ".";
                        }
                    }
                ...
                if (item.getType().equals(Material.BOOK) && custom instanceof CEnchantment) {
                    ...
                    return Success + "You have created an enchanted book with '"
                        + custom.getDisplayName()
                    ...
                    if (custom instanceof CEnchantment) {
                        if (EnchantManager.containsEnchantment(lore, (CEnchantment) custom)) {
                            for (int i = 0; i < lore.size(); i++) {
                                if (EnchantManager.containsEnchantment(lore.get(i),
                                    (CEnchantment) custom)) {
                                    int newLevel = EnchantManager.getLevel(lore.get(i)) + level;
                                    int maxLevel = ((CEnchantment) custom).getEnchantmentMaxLevel();
                                    if (EnchantManager.getLevel(lore.get(i))
                                        == ((CEnchantment) custom).getEnchantmentMaxLevel()) {
                                        return Error
                                    ...
                                    lore.set(i,
                                        custom.getDisplayName() + " " + EnchantManager.intToLevel(
                                            newLevel));
                                    im.setLore(lore);
                                    item.setItemMeta(im);
                                    p.setItemInHand(item);
                                    return (Success + "You have increased your item's level of "
                                        + custom.getDisplayName() + ChatColor.GREEN
                                        + (newLevel == maxLevel ? " to " + maxLevel
                                        : " by " + level)
                                        + "!");
                                }
                            }
                        ...
                        if (number > 0) {
                            for (String s : lore) {
                                if (EnchantManager.containsEnchantment(s)) {
                                    ...
                                    if (number <= 0) {
                                        return (Error
                                            + "You already have the maximum number of Enchantments on your item!");
                                    }
                                }
                            }
                        }
                ...
                if (custom instanceof CEnchantment) {
                    ...
                    Success +=
                        "You have enchanted your item with '" + custom.getDisplayName()
                            + ChatColor.GREEN
                            + "' level " + level + "!";
                    ...
                    Success +=
                        "You have created the item '" + custom.getDisplayName() + ChatColor.GREEN
                            + "'!";
                }
                return Success;
              ...
              if (toChange.startsWith("n")) {
                ...
                if (item.hasItemMeta() && im.hasDisplayName()) {
                  ...
                  if (option.startsWith("c")) {
                    ...
                    try {
                      test = String.valueOf(ChatColor.valueOf(test));
                  ...
                  return Error
                      + "Your item does not have a name to be changed, use '/ce change name set' first.";
                ...
                if (item.hasItemMeta() || item.getItemMeta().hasLore()) {
                  ...
                  if (option.startsWith("c")) {
                    ...
                    if (ChatColor.valueOf(args[3].toUpperCase()) != null) {
                      ...
                      for (String i : lore) {
                        l.add(ChatColor.valueOf(args[3].toUpperCase()) + ChatColor.stripColor(
                            i));
                      }
                      ...
                      item.setItemMeta(im);
                      return Success
                          + "You have successfully changed the color of the item's lore!";
                  ...
                  return Error
                      + "Your item does not have a lore to be changed, use '/ce change lore set' first.";

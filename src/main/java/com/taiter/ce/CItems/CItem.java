...
public abstract class CItem extends CBasic {
  ...
  public abstract boolean effect(Event event,
      Player owner); // The boolean represents whether the cooldown is to be applied or not
  ...
  public void finalizeItem() {
    ...
    try {
      this.itemColor = ChatColor.valueOf(
          Main.config.getString("Items." + getOriginalName() + ".Color").toUpperCase());
    } catch (Exception e) {
      Bukkit.getConsoleSender().sendMessage(
          ChatColor.RED + "[CE] The color of the Custom Item '" + getOriginalName()
    ...
    this.description = new ArrayList<String>(Arrays.asList(
        Main.config.getString("Items." + getOriginalName() + ".Description").split(";")));
    this.cooldownTime = Long.parseLong(
        Main.config.getString("Items." + getOriginalName() + ".Cooldown"));
    this.cost = Double.parseDouble(Main.config.getString("Items." + getOriginalName() + ".Cost"));

    for (String s : description) {
      description.set(description.indexOf(s), ChatColor.GRAY + String.valueOf(ChatColor.ITALIC) + s);
    }
    ...
    try {
      ...
      if (!hasRetriedConfig) {
        ...
        Bukkit.getConsoleSender().sendMessage(
            ChatColor.RED + "[CE] WARNING: Could not configurate the CE-Item '" + getOriginalName()
                + "',");

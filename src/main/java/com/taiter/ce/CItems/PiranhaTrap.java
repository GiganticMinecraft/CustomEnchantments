...
public class PiranhaTrap extends CItem {
  ...
  public PiranhaTrap(String originalName, ChatColor color, String lDescription, long lCooldown,
      Material mat) {
  ...
  public boolean effect(Event event, Player player) {
    if (event instanceof BlockPlaceEvent) {
      ...
      if (!player.hasMetadata("ce.bleed")) {
        ...
        player.sendMessage(
            ChatColor.RED + "You are bitten by piranhas and your bleeding intensifies!");
      }
      ...
      for (int i = 0; i < FishAmount; i++) {
        ItemStack tempFish = new ItemStack(Material.TROPICAL_FISH, 1);
        ItemMeta im = tempFish.getItemMeta();
        im.setDisplayName(String.valueOf(i));
        tempFish.setItemMeta(im);
        Item tempItem = w.dropItem(lo, tempFish);
        tempItem.setPickupDelay(50000);
        tempItem.setVelocity(new Vector(0.02 * (rand.nextInt(2) == 1 ? -1 : 1), 0.4,
            0.02 * (rand.nextInt(2) == 1 ? -1 : 1)));
          ...
          public void run() {
            if (maxTime >= 0) {
              for (Item fish : fishList) {
                ...
                Vector vel = new Vector(0.02 * (rand.nextInt(2) == 1 ? -1 : 1), 0.4,
                    0.02 * (rand.nextInt(2) == 1 ? -1 : 1));
  ...
  public void initConfigEntries() {
    ...
    FishAmount = Integer.parseInt(
        getConfig().getString("Items." + getOriginalName() + ".FishAmount"));
    FishDuration = Integer.parseInt(
        getConfig().getString("Items." + getOriginalName() + ".FishDuration"));
  }

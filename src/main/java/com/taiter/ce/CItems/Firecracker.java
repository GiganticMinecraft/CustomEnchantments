...
public class Firecracker extends CItem {
  ...
  public Firecracker(String originalName, ChatColor color, String lDescription, long lCooldown,
      Material mat) {
  ...
  public boolean effect(Event event, final Player player) {
    if (event instanceof ProjectileHitEvent) {
      ...
      FireworkEffect effect = FireworkEffect.builder().flicker(false).withColor(c1).withFade(c2)
          .trail(true).with(ft).trail(false).build();
      fm.addEffect(effect);

...
public class PricklyBlock extends CItem {
  ...
  public PricklyBlock(String originalName, ChatColor color, String lDescription, long lCooldown,
      Material mat) {
  ...
  public boolean effect(Event event, Player player) {
    if (event instanceof BlockPlaceEvent) {
      ...
      if (!player.getGameMode().equals(GameMode.CREATIVE) && !player.hasPotionEffect(
          PotionEffectType.CONFUSION)) {
        player.damage(Damage);
        player.sendMessage(ChatColor.DARK_GREEN + "A nearbly Block is hurting you!");
        player.addPotionEffect(
            new PotionEffect(PotionEffectType.CONFUSION, NauseaDuration, NauseaLevel));
      }
    }
    return false;
  }
  ...
  public void initConfigEntries() {
    ...
    NauseaLevel =
        Integer.parseInt(getConfig().getString("Items." + getOriginalName() + ".NauseaLevel")) - 1;
    NauseaDuration = Integer.parseInt(
        getConfig().getString("Items." + getOriginalName() + ".NauseaDuration"));
  }

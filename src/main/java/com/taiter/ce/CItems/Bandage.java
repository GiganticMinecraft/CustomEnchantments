...
public class Bandage extends CItem {
  ...
  public Bandage(String originalName, ChatColor color, String lDescription, long lCooldown,
      Material mat) {
      ...
      if (toHeal != null) {
        if (!toHeal.hasMetadata("ce." + getOriginalName())) {
          if (toHeal.getHealth() != toHeal.getMaxHealth()) {
            heal(toHeal);
            player.sendMessage(
                ChatColor.GREEN + "You have applied a bandage on " + toHeal.getName() + ".");
            toHeal.sendMessage(
                ChatColor.GREEN + player.getName() + " has applied a bandage on you.");
            return true;
          } else {
            player.sendMessage(
                ChatColor.RED + "Your target does not have any wounds to apply the bandage to!");
          }
        } else {
  ...
  public void initConfigEntries() {
    TotalHealAmount = Integer.parseInt(
        getConfig().getString("Items." + getOriginalName() + ".TotalHealAmount"));
    TotalHealTime = Integer.parseInt(
        getConfig().getString("Items." + getOriginalName() + ".TotalHealTime"));
    StopAtFullHealth = Boolean.parseBoolean(
        getConfig().getString("Items." + getOriginalName() + ".StopAtFullHealth"));
  ...
  private void heal(final Player p) {
    ...
    p.setMetadata("ce." + getOriginalName(),
        new FixedMetadataValue(main, ChatColor.RED + "You are already using a bandage!"));
    ...
    new BukkitRunnable() {
      ...
      public void run() {
        if (!p.isDead() && localCounter != 0) {
          ...
          if (p.getHealth() + healBursts <= p.getMaxHealth()) {

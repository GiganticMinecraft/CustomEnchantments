...
public class FireworkBattery extends CItem {
  ...
  public FireworkBattery(String originalName, ChatColor color, String lDescription, long lCooldown,
      Material mat) {
    ...
    new BukkitRunnable() {
      ...
      public void run() {
        if (startLoc.getBlock().getType().equals(m) && fireworks > 0) {
          ...
          FireworkEffect effect = FireworkEffect.builder().flicker(true).withColor(c1).withFade(c2)
              .trail(true).with(ft).trail(true).build();
          fm.addEffect(effect);
          fm.setPower(r.nextInt(2) + 2);
          fw.setFireworkMeta(fm);
          Vector v = fw.getVelocity();
          fw.setVelocity(new Vector(v.getX() + (r.nextDouble() - r.nextDouble()) * 0.01, v.getY(),
              v.getZ() + (r.nextDouble() - r.nextDouble()) * 0.01));

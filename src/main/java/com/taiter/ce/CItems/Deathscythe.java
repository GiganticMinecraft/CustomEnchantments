...
public class Deathscythe extends CItem {
  ...
  public Deathscythe(String originalName, ChatColor color, String lDescription, long lCooldown,
      Material mat) {
  ...
  public boolean effect(Event event, final Player player) {
    ...
    if (player.hasMetadata("ce." + getOriginalName())) {
      ...
      if (target == null) {
        player.sendMessage(ChatColor.DARK_GRAY + String.valueOf(ChatColor.ITALIC) + "The Soul escaped!");
        return true;
      }
    ...
    if (e.getAction().toString().startsWith("RIGHT")) {
      ...
      if (ents.isEmpty()) {
        ...
        player.sendMessage(
            ChatColor.DARK_GRAY + String.valueOf(ChatColor.ITALIC) + "There are no souls nearby");
        return true;
      }
      ...
      for (int i = 1; i < Range; i++) {
        ...
        for (Entity ent : ents) {
          ...
          if (Math.abs(entLoc.getBlockX() - loc.getBlockX()) < 2
              && Math.abs(entLoc.getBlockY() - loc.getBlockY()) < 2
              && Math.abs(entLoc.getBlockZ() - loc.getBlockZ()) < 2) {
            EffectManager.playSound(ent.getLocation(), "BLOCK_PISTON_EXTEND", 0.1f, 0.3f);
            player.sendMessage(
                ChatColor.DARK_GRAY + String.valueOf(ChatColor.ITALIC) + "You caught a Soul!");
            player.setMetadata("ce." + getOriginalName(),
                new FixedMetadataValue(main, ent.getEntityId()));
            new BukkitRunnable() {
              @Override
              public void run() {
                if (player.hasMetadata("ce." + getOriginalName())) {
                  player.removeMetadata("ce." + getOriginalName(), main);
                  player.sendMessage(
                      ChatColor.DARK_GRAY + String.valueOf(ChatColor.ITALIC) + "The Soul escaped!");
                }
              }
      ...
      player.sendMessage(ChatColor.DARK_GRAY + String.valueOf(ChatColor.ITALIC) + "You missed!");
      EffectManager.playSound(loc, "BLOCK_PORTAL_TRAVEL", 0.01f, 100f);
  ...
  public void initConfigEntries() {
    ...
    MaximumHookTime = Integer.parseInt(
        getConfig().getString("Items." + getOriginalName() + ".MaximumHookTime"));

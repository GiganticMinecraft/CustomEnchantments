...
public class BearTrap extends CItem {
  ...
  public BearTrap(String originalName, ChatColor color, String lDescription, long lCooldown,
      Material mat) {
  ...
  public boolean effect(Event event, Player player) {
    if (event instanceof BlockPlaceEvent) {
      ...
      Location loc = new Location(player.getWorld(), Integer.parseInt(s[0]), Integer.parseInt(s[1]),
      ...
      if (!b.getType().equals(Material.AIR)) {
        ...
        if (player.hasMetadata("ce.bleed") && player.hasPotionEffect(PotionEffectType.SLOW)
            && player.hasPotionEffect(PotionEffectType.WEAKNESS)) {
          player.sendMessage(
              ChatColor.RED + "You walked into another Bear Trap! Are you kidding me?!");
        } else {
        ...
        player.addPotionEffect(
            new PotionEffect(PotionEffectType.WEAKNESS, WeaknessDuration, WeaknessLevel));
  ...
  public void initConfigEntries() {
    BleedDuration = Integer.parseInt(
        getConfig().getString("Items." + getOriginalName() + ".BleedDuration"));
    SlowLevel =
        Integer.parseInt(getConfig().getString("Items." + getOriginalName() + ".SlowLevel")) - 1;
    SlowDuration = Integer.parseInt(
        getConfig().getString("Items." + getOriginalName() + ".SlowDuration"));
    WeaknessLevel =
        Integer.parseInt(getConfig().getString("Items." + getOriginalName() + ".WeaknessLevel"))
            - 1;
    WeaknessDuration = Integer.parseInt(
        getConfig().getString("Items." + getOriginalName() + ".WeaknessDuration"));

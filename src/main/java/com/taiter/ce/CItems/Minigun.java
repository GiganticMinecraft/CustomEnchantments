...
public class Minigun extends CItem {
  ...
  public Minigun(String originalName, ChatColor color, String lDescription, long lCooldown,
      Material mat) {
      ...
      public void run() {
        if (lArrows > 0) {
          if (player.getItemInHand().hasItemMeta() && player.getItemInHand().getItemMeta()
              .equals(last.getItemMeta())) {
            if (player.getGameMode().equals(GameMode.CREATIVE) || player.getInventory()
                .contains(Material.ARROW, 1)) {
              if (!player.getGameMode().equals(GameMode.CREATIVE)) {
                if (last.getDurability() < 380) {
                  ...
                  player.getWorld()
                      .playEffect(player.getLocation(), Effect.ZOMBIE_DESTROY_DOOR, 10);
              ...
              if (metadata != null) {
                arrow.setMetadata("ce.bow.enchantment",
                    new FixedMetadataValue(getPlugin(), metadata));
              }
  ...
  public void initConfigEntries() {
    ArrowCountPerVolley = Integer.parseInt(
        getConfig().getString("Items." + getOriginalName() + ".ArrowCountPerVolley"));
    ShotsPerSecond = Integer.parseInt(
        getConfig().getString("Items." + getOriginalName() + ".ShotsPerSecond"));
    if (ShotsPerSecond > 20) {
      Bukkit.getConsoleSender().sendMessage(
          ChatColor.RED + "[CE] Warning: The Minigun setting 'ShotsPerSecond' is too high,");
      Bukkit.getConsoleSender().sendMessage(
          ChatColor.RED + "[CE]          the maximum value is 20, which will be used instead. ");
      ShotsPerSecond = 20;
    }
